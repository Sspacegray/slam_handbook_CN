...
其中 `h(·)` 是 measurement prediction function，而噪声 `η` 服从零均值 Gaussian density，测量协方差为 `Σ_R`。这就导出了测量的条件密度 `p(z | p, ℓ)`。

在实际机器人问题中，测量函数 `h(·)` 往往是非线性的。不过尽管如此，它们通常并不难理解，也不难写出。例如，在二维 bearing measurement 中，测量函数就是:

`h(p, ℓ) = atan2(ℓ_y - p_y, ℓ_x - p_x)`

因此最终的概率测量模型 `p(z | p, ℓ)`，就是在这个 `atan2` 观测函数基础上叠加 Gaussian noise 得到的。
...
其中 `g(·)` 是 motion model，`Σ_Q` 是相应维度的协方差矩阵，例如二维平面机器人时常见的是 `3×3`。
...
`δ* = argmin_δ Σ_i ||A_i δ_i - b_i||^2 = argmin_δ ||Aδ - b||^2`

这里，大矩阵 `A` 和右端项 `b` 都是把所有 whitened Jacobians `A_i` 和 whitened prediction errors `b_i` 组合起来得到的。`A` 往往是一个“大而稀疏”的矩阵，并且其块结构直接反映底层 `factor graph` 的结构。
...
其中负梯度给出了 cost function 下降最快的方向，`α` 是步长。对于 `nonlinear least squares`，如果把目标在当前点局部近似成二次型，就能写出精确的梯度表达式。`SD` 方法结构简单，但在接近最优解时通常收敛较慢。
...
当目标函数在局部表现得足够像二次函数，并且初始值较好时，`Gauss-Newton` 往往能实现接近二次收敛。但如果局部二次近似较差，`GN` 更新也可能把估计推得更远，甚至导致发散。
...
该算法从 `SD` 更新方向开始，随后朝 `GN` 方向折过去，但会在 trust region 边界处停止。与 `LM` 一样，`PDL` 也通过 gain ratio 来评估局部线性近似是否可信。如果效果差，就缩小 trust region；如果效果好，就扩大它。由于每次状态更新只需一次矩阵分解，`PDL` 通常比 `LM` 更高效。
...
增量推断可以被看成是对 `Bayes tree` 的一次局部编辑（editing）。这比“增量矩阵分解”那个更抽象的说法要直观得多，也说明了为何 `Bayes tree` 本身可以作为平方根信息矩阵的一种极具意义的稀疏存储结构。

当加入一个新测量时，等价于向系统增加一个新 factor。例如，一个同时涉及两个变量的新二元 factor `ϕ(x_j, x_j')`，只会影响 `Bayes tree` 中包含这两个变量的 cliques 到根之间的路径。位于这些 cliques 下方的子树，以及与这两个变量无关的其他子树，都不会受影响。

因此，更新步骤可以概括为:

1. 把受影响的那部分 `Bayes tree` 临时转换回 `factor graph`
2. 加入新的 factor
3. 重新对这个局部因子图做 elimination，生成新的局部 `Bayes tree`
4. 把原先未受影响的“孤儿子树”重新挂回去

这种局部更新成立的原因在于两个性质:

- `Bayes tree` 中的信息沿着从子到父、最终到根的方向传播
- 一个 factor 只有在它所连接的第一个变量被消去时，才真正进入消元过程

因此，新 factor 不会影响与其变量没有祖先关系的整棵远端子树；只有当它把两条原本独立通向根的路径连接起来时，这些路径才需要重新消元。
