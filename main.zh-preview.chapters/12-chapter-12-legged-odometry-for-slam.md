...
本章讨论 `leg odometry`，也就是如何利用腿式机器人自身的腿部运动学、接触状态以及本体感觉信息来估计机体运动。与轮式里程计相比，腿式里程计更适用于崎岖地形、楼梯、障碍和复杂接触环境，但它所面对的建模问题也明显更难，因为“脚与地面的接触”既会建立，又会断开，还可能打滑、形变甚至失效。

作者先回顾历史背景与预备知识，说明腿式平台中的参考系、状态定义、运动学、动力学与关节感知；然后讨论如何从支撑腿恢复相对位姿和速度，如何判定足端接触状态，如何把这些测量放入滤波器或 `Factor Graph（因子图）` 中，与 `IMU`、camera、`LiDAR` 等外感知传感器共同服务于 `SLAM`；最后总结若干开放挑战与近期趋势。
...
腿式机器人可被视为一个 floating base 加上若干条运动链，也就是腿。书中主要关注最常见的两种平台: 每条腿有 6 个主动自由度、足端为平足的双足机器人，以及每条腿有 3 个主动自由度、足端为点足的四足机器人。作者还假设机体整体是刚体，不考虑脊柱等额外关节。

设 `q` 为机器人全部主动关节位置，则每只脚对应一个 `forward kinematics` 函数 `fk(q)`，它把关节角映射到足端相对 base 的位姿。对平足平台，这个映射包括足端位置和姿态；对 point-foot quadruped，主要关注的是足端位置，因为点足可绕接触点自由转动而不改变关节角。对 `fk(q)` 求时间导数，可得到 Jacobian `J(q)`，它将 joint velocities 映射到足端相对 base 的线速度和角速度。

这些表达式是 `leg odometry` 的基础，因为只要 joint sensing 可用，并且接触假设成立，足端就能对 base motion 提供强几何约束。
...
因此，作者采用与前面章节类似的一阶线性化思路: 在当前测量附近使用 Jacobian 展开 kinematics 函数，把 joint-space 的噪声线性传播到足端相对位姿或速度测量上。对速度测量，还要同时考虑 joint position noise 与 joint velocity noise。这样就能在近似高斯框架下为后续滤波器或因子图构造合理的测量协方差。

### 12.4.2 Factor Graph（因子图）平滑

为了保证 locomotion control 的实时性，腿式机器人过去长期依赖 `EKF`、`UKF` 或 invariant `EKF` 等滤波器。这些滤波器通常把高频 `IMU` 和运动学观测结合起来，给控制器提供紧密闭环所需的高频状态。

但从更一般的估计角度看，`Factor Graph` 方法更具表达力，因为它不强迫我们区分“过程模型”和“测量模型”，而是统一将变量之间的关系写成因子。当窗口只包含两个连续状态时，因子图几乎就像一个滤波器；而当窗口扩大时，图优化就可以同时校正一段历史状态。若再结合预积分技术，就能在不显著增加变量数量的前提下，跨更长时间范围整合高频测量。

#### 12.4.2.1 接触预积分

接触预积分的想法与 `IMU preintegration` 非常相似。对于 humanoid，如果某条支撑腿在一段时间内持续保持 stance，则其相对接触运动增量可以被压缩成一个连接两个状态的因子。作者引用 Hartley 等人的做法，将其写成一个包含 prior factor、预积分 `IMU factor`、forward kinematics factor 和 contact preintegration factor 的因子图。

其中，`forward kinematics factor` 用于把足端接触系姿态与机器人 base 姿态通过支撑腿的 forward kinematics 联系起来。把上一节推导的 encoder noise propagation 代入相对位姿测量后，就能得到这类因子的 residual 与 covariance；其本质约束是“机器人 base 与接触坐标系之间的相对位姿，应与由关节读数推得的运动学结果一致”，而不确定性则来自关节编码器噪声经 Jacobian 传播后的结果。

`Contact preintegration factor` 则进一步约束两个时刻之间接触点的演化。理想情况下，若 stance leg 没有 slip，则接触框架的旋转和位置在支撑阶段应保持不变；实际中，接触点速度会受到 slip 等效应影响，因此作者将其建模为高斯噪声。于是，在两个连续状态 `xi` 和 `xj` 之间，接触预积分残差可以写成“旋转部分接近恒等、平移部分接近零”的 stacked residual，其协方差则由接触角速度协方差和线速度协方差在时间区间 `Δtij` 上积分得到。

作者也明确指出，这一因子有重要限制: 它只在同一条 stance leg、且接触模式没有切换时严格成立。因此它无法直接覆盖腿部切换动力学，后续工作才专门针对 contact frame switch 做了扩展。
...
若再假设在 `ti` 到 `tj` 之间 base 线速度近似常值，就能把这些速度测量预积分为一个相对位移量。书中的表达形式，是把每个小时间段内经姿态校正后的线速度累加起来，得到预积分位移 `Δtij`，同时将噪声压缩为一个预积分速度噪声项。

在此基础上，就能构造 `velocity preintegration factor`。它的残差本质上比较的是: 当前两状态间的相对平移 `Ri^T(tj - ti)`，是否与由腿式里程计速度预积分得到的 `Δtij` 一致；其协方差则由各小时间段速度噪声传播并叠加得到。这样，速度预积分因子便可与 `IMU` 因子一同进入图中，为四足平台提供更直接的平移约束。
...
本章前面几乎所有 `leg odometry` 方程都默认机器人是完美刚体。但现实中，腿部连杆会因载荷而发生弹性弯曲，这会让 forward kinematics 预测的足端理想位置与真实位置产生系统偏差。对双足平台而言，由于腿更长，这一问题往往更严重。

一种思路是显式建模腿部弹性，将负载与形变的关系纳入估计；但这需要非常仔细的结构建模，通常复杂且泛化性差。更常见的路线，是在腿部各连杆上额外布置 `IMU`，通过更多本体感觉信息来估计真实 link orientation，从而部分修正纯关节读数的不足。
