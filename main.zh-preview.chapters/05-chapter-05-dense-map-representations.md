...
对 `LiDAR` 而言，距离测量由激光发射、反射与回波检测得到，通过发射时间 `temit` 和接收时间 `tdetect`，可以计算 range `r`。若把单束激光扩展为沿某种扫描模式运动的多束激光，就可形成二维或三维点集合，也就是 `point cloud`。对机械旋转式 `LiDAR`，原始测量也常被整理成 `range image`。

对 `RGB-D camera` 而言，传感器通常同时输出:

- `RGB image`
- 与之同分辨率的 `depth map`

早期 `RGB-D` 相机常依赖结构光，主要适合室内；较新的 `TOF` 或改进型红外方案则对室外场景更友好。

### 5.1.2 转换为点云

有了 range image 或 depth map，再结合传感器内参，就可以把测量转换成 point cloud `P = {p1, ..., pN}`。点是 dense mapping 中最基本的构件。

对 `LiDAR`，每条 beam 通常带有已知的 azimuth 与 inclination，因此可通过球坐标变换得到三维点坐标。对 `RGB-D camera`，则常使用 pinhole camera model，根据像素位置与深度，把图像坐标转换到相机坐标系。
...
`Occupancy maps` 是最经典、也最广泛使用的稠密地图表示之一。其基本思想是，把空间划分为一系列 cells 或 voxels，并为每个单元估计“被占据”的概率。最简单时，这实际上就是一个二分类问题: occupied 或 free。

在给定传感器 measurements 与 poses 的前提下，occupancy mapping 可以通过著名的 `log-odds` 形式进行高效增量更新。它的主要优势有两点:
...
这意味着传统 occupancy grids 无法很好建模空间相关性，也难以在整张地图里灵活调整分辨率。
...
- 是表示 `surface` 还是 `volume`
- 是 `explicit` 还是 `implicit`

对 surface 而言，最直接的 explicit 表示包括:

- point clouds
- meshes
- surfels

这些表示直接描述物体边界本身，适合渲染、可视化、CAD、AR 等任务。对 volume 而言，explicit 表示典型包括 occupancy voxels 与 distance-based voxels；而 `Gaussian Processes`、`Hilbert maps` 等则代表了更偏 `implicit` 的连续函数式建模。
