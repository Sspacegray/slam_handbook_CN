...
- Open issues:
  ...
  - `2026-04-07` 再次直接对照 `main.pdf` / `/tmp/slam-main-20260403.txt` 后，已补回第 `5` 章系统性缺漏：`Figure 5.1-5.11` captions、`Table 5.1` / `Table 5.2` 标题与表格结构、`5.1.1` 关于本章仅以简单 `2D` 与机械旋转式 `3D LiDAR` 为例的脚注、`5.2.1` 关于 occupancy mapping 不限于 grid-based methods 的脚注、`5.3.3.4` 关于 `VDB` 含义的脚注；同时补回第 `6` 章 `Figure 6.1-6.4` captions
  - `2026-04-07` 上述修订已重新同步到 `/home/suja/slam-handbook-public-release/context/Chapter5.md`、`/home/suja/slam-handbook-public-release/context/Chapter6.md`，并保留统一全文文件与章节拆分文件双模式一致
  - `2026-04-07` 继续用“源文 caption 编号必须在译文中显式出现”的严格规则复核 `Chapter 1-12` 后，又补回此前漏掉的图注编号：`Figure 1.8`、`Figure 1.10`、`Figure 1.11`、`Figure 1.12`、`Figure 1.13`、`Figure 2.1`、`Figure 2.2`、`Figure 2.4`
  - `2026-04-07` 上述图注补回后，已重新对 `/tmp/slam-main-20260403.txt` 的 `Chapter 1-12` 全部 `Figure/Table` caption 编号执行显式匹配校验；当前统一译文中不再存在“源文有 caption 编号而译文没有对应 `图/表/Figure/Table` 标号”的剩余项
  - `2026-04-07` 继续针对短叶子小节与脚注式说明做语义级复核后，又补回或收紧了以下位置的实质内容：`1.4.3` 中 `LM` 的 Bayesian 解释；`3.1` 对“本章算法适用于 manifold-valued SLAM”的脚注说明；`3.3.3` 的 `B-R duality` 下交替最小化子问题 `(3.21)`、两步更新 `(3.22)-(3.23)` 与 `truncated quadratic` 分段权重；`3.3.4` 的 `GNC` 三步流程 `(3.26)-(3.27)` 与 `μ` 调度方向；`4.3.2` 的 `group action` Jacobians `(4.37a)(4.37b)` 及 `Robot Arm` 示例 `(4.38)(4.39)`；`5.4` 表 `5.1` 后关于混合空间抽象的限定说明；`6.1.2.1` 关于 simple graph 仅为简化记号的脚注；`6.2.1→6.2.2` 过渡中 FIM 结构分析对主动感知的意义；`6.2.2` 中 reduced incidence matrix 的锚定含义；`7.3.1.2` photometric calibration 只需在尺度上成立的说明；`7.3.7` 初始化对式 `(7.3)` 非线性迭代收敛的作用；`7.3.8` 地图表示的用途、三重折中与第 `5` 章引用；`10.7.3` 位置误差按平均场景深度或总行驶距离归一化的评估约定；`11.2.3` 高级预积分技术的完整引言；`11.3.3` 退化运动成立所需的 `||p_f^c|| >> ||p_b^c||` 条件；`12.1.6` 关节感知面向 planning/control 的背景说明
  - `2026-04-07` 上述修订后，统一全文与 `/home/suja/slam-handbook-public-release/context/Chapter1.md` 到 `Chapter12.md` 已再次重新同步
  - 仍需继续人工核验的高风险内容包括：`broader exact audit remains in progress for any residual short leaf sections or footnote-like omissions`; `semantic-level paragraph-by-paragraph comparison is still needed in selected short sections even though explicit Figure/Table caption numbering is now covered`; `do not yet treat the handbook as final-complete solely from the status file`
- Resume instruction:
