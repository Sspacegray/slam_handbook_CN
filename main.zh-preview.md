...
图 `6.1` 给出了 `SLAM` 最大似然估计中的局部极小值示例。该图展示了真实世界 pose-graph `SLAM` 问题 `(6.9)` 的若干局部极小值，其中机器人在 `(a)` 所示的多层停车楼中运动；`(b)` 是用 `SE-Sync` [936] 算得的正确解，也就是全局最优解；`(c)` 与 `(d)` 则是从随机采样初值出发的局部优化方法得到的次优局部极小值示例。（`(a)` 面板转引自 [400]。）
...
为便于建模，作者先引入 `pose graph`。这里可将其看作 factor graph 的一个特殊情形：变量节点是 poses，factor 则关联成对 poses。具体地，令 `G = (V, E)` 为一个简单无向图，其中节点 `i ∈ V` 与未知位姿 `T_i` 一一对应，边 `{i, j} ∈ E` 与可用测量一一对应。这里假设 `G` 为 simple graph 只是为了简化记号，避免还要区分多条平行边，并不是后续方法本身的必要前提。作者进一步假设 `G` 是连通的；若图不连通，则 `PGO` 会分解为若干独立的连通分量子问题。接着，对每条无向边赋予一个方向，就得到 `pose graph` `G^→ = (V, E^→)`。按照约定，测量 `T̃_ij` 表示在位姿 `T_i` 的坐标系下对位姿 `T_j` 的带噪声观测，因此它对应一条从 `i` 指向 `j` 的有向边。

图 `6.2` 展示了 pose graph 的例子。在该图中，顶点与待估计的未知位姿 `T_i = (t_i, R_i) ∈ SE(d)` 一一对应，而有向边则与它们之间的相对位姿带噪测量 `T̃_ij ≈ T_i^(-1) T_j` 一一对应。
...
图 `6.3` 给出了 pose-graph optimization 基准数据集上的全局最优解结果（引自 [936]）。
...
图 `6.4` 中，`(a)` 展示了一个简单 landmark-based `SLAM` 问题中的局部最小值与全局最小值示例；`(b)` 展示了 `(a)` 中示例的运行时间，且运行时间会随地图规模线性增长，其中瓶颈是数据矩阵 `Q̃` 的构造（蓝色）；`(c)` 则通过证书矩阵的余秩（当该指标为 `3` 时表示精确）研究了松弛的 exactness。图中测量噪声以一个基准标准差为起点：平移为 `0.866` 米，旋转为 `0.573` 度，并按图里的 `Noise Level` 倍率缩放。总体而言，随着 map points 数量增加、pose-to-map connectivity 提升，问题保持 exact 的噪声范围也会增大。该图来自 [465]（©2023 IEEE）。
...
这里作者还特别说明：即使不去分析 Jacobian 的内部结构，我们也仍然可以在当前估计处数值计算 `FIM`，并用它评估一个给定 `SLAM` 问题的估计精度；但若更深入理解 `FIM` 的结构，就能够进一步预测对 `SLAM` 问题的修改，例如由机器人主动采样驱动的观测变化，会如何影响未来估计不确定性，因此这类结构分析对 active perception 与设计任务很有价值。
...
这里的 reduced incidence matrix 是从普通 incidence matrix 中删去与锚定位姿对应的那些行得到的；在 `SLAM` 里，通常会把第一个位姿锚定为世界坐标系原点，以消除仅含相对测量时由问题对称性带来的 gauge symmetry。
